...
from vllm import SamplingParams, EngineArgs, RequestOutput

from my_llm_engine import MyLLMEngine
...
def run_vllm(
    ...
    print("start load")
    llm = MyLLMEngine.from_engine_args(EngineArgs(**vllm_params))
    print("end load")
    ...
    for i, prompt, output_len in requests:
        para = SamplingParams(
            n=1,
            # best_of=None,
    ...
    return end - start, input_num_tokens, output_num_tokens, llm.log
...
def main_vllm(args: argparse.Namespace):
    ...
    params = {
        ...
        # "max_parallel_loading_workers": num_gpu,
        "block_size": 16,
        "swap_space": 4,
    ...
    elapsed_time, input_num_tokens, output_num_tokens, running_log = run_vllm(requests, params)
